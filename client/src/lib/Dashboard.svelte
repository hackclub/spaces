<script>
  ...
  let showSafariNotSupported = false;
  let acknowlegeSafariNotSupported = false;

  onMount(() => {
    loadClubData();
    loadSpaces();

    showSafariNotSupported = /^((?!chrome|android).)*safari/i.test(
      navigator.userAgent,
    );
  });
  ...
  function togglePasswordReveal(spaceId) {
    if (revealedPasswords.has(spaceId)) {
      revealedPasswords.delete(spaceId);
    } else {
      revealedPasswords.add(spaceId);
    }
  ...
  function openSpace(event, space) {
    ...
    fetch(loginUrl.toString(), {
      ...
      .catch((err) => {
        console.error(
          "Auto-login failed, falling back to the login page:",
          err,
        );
      })
      .finally(() => {
...
{#if showSafariNotSupported == true && acknowlegeSafariNotSupported == false}
  <div class="safari-not-supported">
    <p>Safari is not a supported browser</p>
    <button
      on:click={() => {
        acknowlegeSafariNotSupported = true;
      }}
      class="btn-primary">Ignore</button
    >
  </div>
{/if}
...
<div class="dashboard">
  ...
  <div class="dashboard-content">
    ...
    {#if showCreateForm}
      <div class="create-form">
        ...
        {#if newSpaceType !== "kicad" && newSpaceType !== "blender" && newSpaceType !== "freecad"}
          <div class="form-group">
            <label class="form-label" for="homeDir"
              >Home Directory (optional)</label
            >
            <p class="password-info">
              The home folder name. Must live under
              <code>/config</code> so it persists. Defaults to
              <code>/config/workspace</code>. If you are using hackatime, you
              should put different folder names for each project.
            </p>
            <input
              class="form-input"
              id="homeDir"
            ...
            />
            <p class="password-info">
              This is what's used to identify this space later on your
              dashboard.
            </p>
          </div>
        {:else}
        ...
        <button class="btn-primary" on:click={createSpace} disabled={loading}>
    ...
    <div class="spaces-list">
      ...
      {:else}
        <div class="spaces-grid" class:list-view={viewMode === "list"}>
          {#each filteredSpaces as space}
            <div class="space-card" class:list-mode={viewMode === "list"}>
              <div class="space-header">
                <h4 class="space-type">{space.type}</h4>
                <div class="space-header-right">
                  <button
                    class="fav-btn"
                    class:active={space.is_favorite}
                    on:click={() => toggleFavorite(space.id)}
                    title={space.is_favorite
              ...
              <div class="space-info">
                ...
                <p>
                  ...
                  {formatLastOpened(space.last_opened_at)}
                </p>
                {#if space.type === "code-server" && space.password}
                  <p class="space-password">
                    <strong>Password:</strong>
                    <code
                      >{revealedPasswords.has(space.id)
                        ? space.password
                        : "••••••••"}</code
                    >
                    <button
                      type="button"
                      class="link-button"
                      on:click={() => togglePasswordReveal(space.id)}
                    >
                      {revealedPasswords.has(space.id) ? "Hide" : "Show"}
                    </button>
                    <button
                      type="button"
                      class="link-button"
                      on:click={() => copyPassword(space.password)}
                    >
                      Copy
                    </button>
                    <button
                      type="button"
                      class="link-button"
                      on:click={() => openPasswordEditor(space.id)}
                    >
                      Change
                    </button>
                  </p>
                  {#if passwordEditor.spaceId === space.id}
                    <div class="password-editor">
                      ...
                      {#if passwordEditor.error}
                        <p class="password-editor-error">
                          {passwordEditor.error}
                        </p>
                      {/if}
                  ...
                  {#if space.running || space.status?.toLowerCase() === "running"}
                    {#if space.access_url}
                      ...
                      <button
                        ...
                        on:click={() =>
                          copySpaceUrl(space.id, space.access_url)}
                      ...
                      >
                        {#if copyFeedback[space.id]}
                          ...
                          >
                            <path
                              d="M10.707 14.293C10.317 14.683 10.317 15.317 10.707 15.707L15.293 20.293C15.683 20.683 16.317 20.683 16.707 20.293L28.293 8.70699C28.683 8.31699 28.683 7.68299 28.293 7.29299L27.957 6.95699C27.567 6.56699 26.933 6.56699 26.543 6.95699L16 17.5L12.457 13.957C12.067 13.567 11.433 13.567 11.043 13.957L10.707 14.293V14.293Z"
                            />
                          </svg>
                        {:else}
                          <svg
                          ...
                          >
                            <path
                              d="M10 14C10 13.448 10.448 13 11 13H22C22.552 13 23 13.448 23 14C23 14.552 22.552 15 22 15H11C10.448 15 10 14.552 10 14Z"
                            />
                            <path
                              d="M10 18C10 17.448 10.448 17 11 17H22C22.552 17 23 17.448 23 18C23 18.552 22.552 19 22 19H11C10.448 19 10 18.552 10 18Z"
                            />
                            <path
                              d="M10 22C10 21.448 10.448 21 11 21H19C19.552 21 20 21.448 20 22C20 22.552 19.552 23 19 23H11C10.448 23 10 22.552 10 22Z"
                            />
                            <path
                              d="M17.96 3.93119C17.9648 3.95466 17.9674 3.97763 17.9679 4H20C20.552 4 21 4.448 21 5V7C21 7.552 20.552 8 20 8H12C11.448 8 11 7.552 11 7V5C11 4.448 11.448 4 12 4H14.0321C14.224 3.02719 15.034 2.31519 16 2.31519C16.965 2.31519 17.776 3.02719 17.96 3.93119Z"
                            />
                            <path
                              d="M6 17C6 9.14915 6.42432 6.39217 10 5.45705V7C10 7.18211 10.0244 7.35854 10.07 7.52622C9.563 7.71106 9.28679 7.92979 9.099 8.155C8.779 8.539 8.458 9.249 8.255 10.752C8.046 12.283 8 14.274 8 17C8 19.725 8.046 21.717 8.255 23.248C8.458 24.751 8.779 25.461 9.099 25.845C9.375 26.176 9.842 26.493 10.952 26.709C12.164 26.95 13.744 27 16 27C18.256 27 19.836 26.95 21.048 26.709C22.158 26.493 22.625 26.176 22.901 25.845C23.221 25.461 23.542 24.751 23.744 23.248C23.954 21.717 24 19.725 24 17C24 14.274 23.954 12.283 23.744 10.752C23.542 9.249 23.221 8.539 22.901 8.155C22.7132 7.92979 22.437 7.71106 21.93 7.52622C21.9756 7.35854 22 7.18211 22 7V5.45705C25.5757 6.39217 26 9.14915 26 17C26 28 25.167 29 16 29C6.833 29 6 28 6 17Z"
                            />
                          </svg>
                        {/if}
                      </button>
