        ...
        <div class="space-card"
         class:list-mode={viewMode === "list"}>
          ...
          <div class="space-info">
            ...
            <p><strong>Directory:</strong> {space.volume_path || "N/A"}</p>
            {#if space.type === "code-server" && space.password}
              <p class="space-password">
                <strong>Password:</strong>
                <code>{revealedPasswords.has(space.id) ? space.password : "••••••••"}</code>
                <button type="button" class="link-button" on:click={() => togglePasswordReveal(space.id)}>
